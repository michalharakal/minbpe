...
/**
 * Docker container implementation for the Kotlin minbpe tokenizer.
 * Provides access to the Kotlin Multiplatform implementation via containerized CLI.
 */
class KotlinContainer : ContainerInterface {
    
    companion object {
        private const val CONTAINER_NAME = "minbpe-kotlin"
        private const val IMAGE_NAME = "minbpe-kotlin:latest"
    ...
    override suspend fun isHealthy(): Boolean {
        return try {
            if (!isContainerRunning(CONTAINER_NAME)) {
                ...
                val result = executeInContainer(CONTAINER_NAME, listOf("java", "-jar", "/app/cli.jar", "--help"))
                result.exitCode == 0
            }
        } catch (e: Exception) {
            false
    ...
    override suspend fun start() {
        if (!containerExists(CONTAINER_NAME)) {
            ...
            if (result.exitCode != 0) {
                throw RuntimeException("Failed to create Kotlin container: ${result.error}")
            }
        } else if (!isContainerRunning(CONTAINER_NAME)) {
            // Start existing container
            if (!startContainer(CONTAINER_NAME)) {
                throw RuntimeException("Failed to start Kotlin container")
            }
        }
    }
